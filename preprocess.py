...
if __name__ == '__main__':
    ...
    def save(file, idx, label):
        ...
        df.loc[idx, NOTES_COL].to_pickle(os.path.join(check_sys_path(), "%s_idx.pkl" % label))
        df.loc[idx, MEDICINE_COL].to_csv(os.path.join(check_sys_path(), "%s_label.csv" % label), index=False)

    with open(os.path.join(check_sys_path(), "data_stat.txt"), "w") as f:
